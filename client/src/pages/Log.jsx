...
const INITIAL_GOOD = { morning: false, exercise: false, reading: false, prayer: false, coding: false, sleep: false, diet: false, hydration: false };
const INITIAL_BAD  = { social: false, procrastination: false, junk: false, late: false, fajr: false };

const GOOD_LABELS = { morning: '🌅 Morning Routine', exercise: '🏃 Exercise', reading: '📖 Quran & Reading', prayer: '🤲 Daily Prayer', coding: '💻 Coding', sleep: '😴 Quality Sleep', diet: '🥗 Healthy Diet', hydration: '💧 Hydration' };
const BAD_LABELS  = { social: '📱 Social Media', procrastination: '⏰ Procrastination', junk: '🍔 Junk Food', late: '🌙 Sleeping Late', fajr: '🕌 Missing Fajr' };
...
export default function Log() {
  ...
  const [customStates, setCustomStates] = useState({});    // { [habitId]: boolean }
  const [loading,      setLoading]      = useState(true);
  const [saving,       setSaving]       = useState(false);
  const [existingId,   setExistingId]   = useState(null);
  const today = TODAY();

  // Load today's entry + custom habits in parallel
  const loadData = useCallback(async () => {
    try {
      ...
      if (entryRes.status === 'fulfilled') {
        ...
        // Restore custom habit states from saved entry
  ...
  const toggleCustom = (id) => setCustomStates(p => ({ ...p, [id]: !p[id] }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await saveEntry({ date: today, good, bad, mood, notes, customHabits: customHabitsPayload });
      toast.success(existingId ? 'Entry updated ✓' : 'Entry saved ✓');
      markLogged();           // dismiss notification banner
      navigate('/dashboard');
    } catch {
  ...
  if (loading) return <div className="page-loader">Loading today's log…</div>;
    ...
    <main className="page">
      ...
      <div className="live-score-card">
        <div>
          ...
          <div className="text-muted" style={{ fontSize: 12, marginTop: 2 }}>Updates as you check habits</div>
        </div>
        <div className="live-score-value" style={{ color: gradeColor(score) }}>{score}</div>
      </div>

      <div className="log-form">
        {/* ── Good Habits ── */}
        <div className="log-section">
          <div className="section-title">✅ Good Habits <span style={{ color: 'var(--text-muted)', textTransform: 'none', fontWeight: 400 }}>— check what you did</span></div>
          <div className="habits-grid">
            {Object.keys(INITIAL_GOOD).map(key => (
              <label key={key} className={`habit-toggle ${good[key] ? 'checked' : ''}`} onClick={() => toggleGood(key)}>
                <input type="checkbox" checked={good[key]} readOnly />
                {GOOD_LABELS[key]}
              </label>
            ))}
          </div>
          ...
          {customGood.length > 0 && (
            <>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginTop: 8, letterSpacing: '0.04em' }}>CUSTOM GOOD HABITS</div>
              <div className="habits-grid">
                {customGood.map(h => (
                  <label key={h._id} className={`habit-toggle ${customStates[h._id] ? 'checked' : ''}`} onClick={() => toggleCustom(h._id)}>
                    <input type="checkbox" checked={customStates[h._id] ?? false} readOnly />
                    {h.emoji} {h.name}
                  </label>
                ))}
              </div>
        ...
        {/* ── Bad Habits ── */}
        <div className="log-section">
          <div className="section-title">❌ Bad Habits <span style={{ color: 'var(--text-muted)', textTransform: 'none', fontWeight: 400 }}>— check what you did</span></div>
          <div className="habits-grid">
            {Object.keys(INITIAL_BAD).map(key => (
              <label key={key} className={`habit-toggle bad ${bad[key] ? 'checked' : ''}`} onClick={() => toggleBad(key)}>
                <input type="checkbox" checked={bad[key]} readOnly />
                {BAD_LABELS[key]}
              </label>
            ))}
          </div>
          ...
          {customBad.length > 0 && (
            <>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginTop: 8, letterSpacing: '0.04em' }}>CUSTOM BAD HABITS</div>
              <div className="habits-grid">
                {customBad.map(h => (
                  <label key={h._id} className={`habit-toggle bad ${customStates[h._id] ? 'checked' : ''}`} onClick={() => toggleCustom(h._id)}>
                    <input type="checkbox" checked={customStates[h._id] ?? false} readOnly />
                    {h.emoji} {h.name}
                  </label>
                ))}
        ...
        <div className="log-section">
          ...
          <div className="mood-picker">
            {[1, 2, 3, 4, 5].map(n => (
              <button key={n} type="button" className={`mood-btn ${mood === n ? 'selected' : ''}`} onClick={() => setMood(n)}>
                {['😞','😕','😐','🙂','😄'][n - 1]}
              </button>
            ))}
          </div>
